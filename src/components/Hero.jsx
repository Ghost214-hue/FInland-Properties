...
export default function Hero() {
    ...
    // Derive whether to show the poster column
    const hasPoster = !!data.promoPoster;
        ...
        <>
            ...
            >
                ...
                <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16 md:pt-24 md:pb-20">
                    ...
                    {/*
                        Two-column when a poster exists; single column otherwise.
                        The grid collapses to one column on mobile regardless.
                    */}
                    <div className={`grid grid-cols-1 gap-10 xl:gap-16 items-center ${hasPoster ? 'lg:grid-cols-2' : ''}`}>

                        {/* ════════════════════════════════
                            LEFT — hero copy
                        ════════════════════════════════ */}
                        <div className={`flex flex-col gap-4 sm:gap-6 items-start text-left ${!hasPoster ? 'max-w-3xl' : ''}`}>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                ...
                                >
                                    {data.ctaPrimaryLabel || 'View Available Plots'}
                                ...
                                >
                                    {data.ctaSecondaryLabel || 'Book a Site Visit'}
                                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        ...
                        {/* ════════════════════════════════
                            RIGHT — Promo Poster
                            Only rendered when `promoPoster` is set in Sanity.
                            To update: go to the Hero document in your Sanity
                            Studio and upload a new Promo Poster image.
                        ...
                        {hasPoster && (
                            <div className="flex items-center justify-center lg:justify-end">
                                <div className="relative">
                                    ...
                                    {/* Ambient glow */}
                                    <div className="absolute -inset-6 rounded-3xl bg-gradient-to-br from-emerald-400/25 via-green-300/15 to-teal-400/20 blur-3xl pointer-events-none" />

                                    {/* Poster frame */}
                                    <div className="relative rotate-1 hover:rotate-0 transition-transform duration-500 ease-out max-w-[300px] sm:max-w-[340px] md:max-w-[380px] lg:max-w-[420px] w-full">

                                        {/* White mount border */}
                                        <div className="p-2.5 sm:p-3 bg-white/90 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl shadow-green-900/40 ring-1 ring-green-100">
                                            <img
                                                src={urlFor(data.promoPoster).url()}
                                                alt={data.promoPosterAlt || 'Current Promotion'}
                                                className="w-full h-auto object-cover rounded-xl sm:rounded-2xl block"
                                            />
                                        </div>

                                        {/* Inner vignette */}
                                        <div className="absolute inset-2.5 sm:inset-3 rounded-xl sm:rounded-2xl shadow-[inset_0_0_20px_rgba(0,0,0,0.12)] pointer-events-none" />
                                    </div>

                                    {/* Floating label pill — top-left */}
                                    <div className="absolute -top-3.5 -left-3 z-10 flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-green-100 shadow-md shadow-green-900/10">
                                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                        <span className="text-[0.6rem] font-black uppercase tracking-widest text-green-800">
                                            {data.promoPosterLabel || 'Current Offer'}
                                        </span>
                                    </div>

                                    {/* Enquire CTA pill — bottom-center */}
                                    <div className="absolute -bottom-5 left-0 right-0 flex justify-center z-10">
                                        <a
                                            href={data.promoPosterCtaHref || '#contact'}
                                            className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 rounded-full text-[0.65rem] font-black uppercase tracking-widest text-white shadow-lg shadow-green-700/35 hover:-translate-y-0.5 transition-all duration-200 whitespace-nowrap"
                                        >
                                            {data.promoPosterCtaLabel || 'Enquire About This Offer'}
